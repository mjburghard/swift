...
static bool parseDifferentiableAttributeArgument(Parser &P,
  ...
  // If the next token is not a `(` or `@`, then the matched '( <identifier> )'
  // is actually the parameter type list, not an argument to '@differentiable'.
  if (P.Tok.isNot(tok::l_paren, tok::at_sign))
    return false;
